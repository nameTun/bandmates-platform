...
export class VocabularyController {
  ...
  async getWordAnalysisAi(
    ...
    if (realUserId) {
      try {
        ...
        console.warn(
          `[Word-Analysis] Không lấy được profile cho user ${realUserId}, dùng mặc định.`,
        );
      }
    }
    const usage = await this.vocabularyService.getWordAnalysisAi(
      word,
      realUserId,
      ip,
      visitorId,
  ...
  async getExampleWordFamilyAi(
    ...
    if (realUserId) {
      try {
        ...
        console.warn(
          `[Word-Family] Không lấy được profile cho user ${realUserId}, dùng mặc định.`,
        );
      }
    }
    const usage = await this.vocabularyService.getExampleWordFamilyAi(
      word,
      realUserId,
      ip,
      visitorId,
  ...
  async toggleSave(@Body('word') word: string, @GetUser() user: User) {
