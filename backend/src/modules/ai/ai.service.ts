...
export class AiService {
  ...
  private async withRetry<T>(
    ...
    try {
      ...
      if (
        ...
        console.warn(
          `Gemini 503 detected, retrying in ${delay}ms... (${retries} left)`,
        );
  ...
  async generateWithFallback(
    ...
    for (let i = 0; i < modelList.length; i++) {
      ...
      if (!isAvailable && !isLastModel) {
        console.warn(
          `[Quota Fallback] Model ${modelName} đã hết hạn mức trong DB. Đang thử model tiếp theo...`,
        );
        continue;
      }
      ...
      try {
        ...
        lastError = error;
        const status =
          error.status || (error.message?.includes('429') ? 429 : 0);
        ...
        if (status === 429) {
          ...
          if (!isLastModel) {
            console.warn(
              `[Gemini Fallback] Model ${modelName} hết hạn mức (429). Đang chuyển sang model tiếp theo...`,
            );
            continue;
          }
        }
        ...
        console.error(
          `[Gemini Error] Model ${modelName} gặp lỗi:`,
          error.message,
        );
  ...
  private async checkAvailability(modelName: string): Promise<boolean> {
    ...
    if (usage.currentRPD >= limits.rpd) {
      console.warn(
        `[Quota] Model ${modelName} đã hết hạn mức ngày (RPD: ${usage.currentRPD}/${limits.rpd})`,
      );
      return false;
    }
    ...
    if (usage.currentRPM >= limits.rpm) {
      console.warn(
        `[Quota] Model ${modelName} đang bận (RPM: ${usage.currentRPM}/${limits.rpm})`,
      );
      return false;
    }
