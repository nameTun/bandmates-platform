    ...
    <aside className="w-64 bg-white border-r border-slate-200/80 flex flex-col justify-between h-screen sticky top-0 flex-shrink-0">
      ...
      <div>
        ...
        <nav className="flex flex-col gap-1 p-3 mt-1">
          {/* Admin Back-link: Chỉ hiển thị cho Admin khi ở giao diện User */}
          {user?.role === 'admin' && (
            <NavLink
              to="/admin"
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-bold bg-slate-900 text-white shadow-lg shadow-slate-200 mb-4 hover:bg-slate-800 transition-all group"
            >
