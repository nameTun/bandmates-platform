...
const CategoryManagement: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      message.error('Không thể tải dữ liệu phân loại');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async () => {
    if (!formName.trim()) return message.warning('Vui lòng nhập tên');
    try {
      setSubmitting(true);
      if (activeTab === 'TOPICS') {
        await topicService.createTopic({
          ...
          taskType: TaskType.TASK_2 // Mặc định cho Task 2
        });
      } else {
  ...
  const handleDelete = (id: string, name: string, count: number) => {
    ...
    Modal.confirm({
      title: isTopic ? 'Xoá vĩnh viễn Chủ đề?' : 'Xác nhận xoá dạng bài?',
      icon: <ExclamationCircleOutlined className={isTopic ? 'text-red-600' : 'text-red-500'} />,
      content: (
        <div className="mt-3">
          <p className="text-slate-600">Bạn đang chuẩn bị xoá {isTopic ? 'chủ đề' : 'dạng bài'} <span className="font-bold text-slate-800">"{name}"</span>.</p>

          {count > 0 && (
            <div className={`mt-2 p-3 ${isTopic ? 'bg-red-600' : 'bg-red-50'} border ${isTopic ? 'border-red-700' : 'border-red-100'} rounded-lg shadow-sm`}>
              <p className={`${isTopic ? 'text-white' : 'text-red-700'} text-xs font-semibold leading-relaxed`}>
                {isTopic ? (
                  <>
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="text-[14px]">⚠️</span>
                      <span className="font-black uppercase text-[11px] tracking-wider">Cảnh báo nguy hiểm</span>
                    </div>
                    Hiện có <span className="underline font-black">{count} đề bài</span> thuộc chủ đề này.
                    Khi xoá chủ đề, <span className="font-black uppercase">Toàn bộ đề bài đó sẽ bị xoá sạch</span> khỏi Database (CASCADE)!
                  </>
                ) : (
                  <>
          ...
          <p className={`mt-3 ${isTopic ? 'text-red-700' : 'text-red-500'} text-[11px] font-bold italic uppercase tracking-wider animate-pulse`}>
            {isTopic ? 'Hành động này cực kỳ nguy hiểm và không thể hoàn tác!' : 'Thao tác này không thể hoàn tác!'}
          </p>
        </div>
      ),
      okText: isTopic ? 'Tôi chấp nhận rủi ro, hãy xoá' : 'Tôi hiểu, hãy xoá',
      ...
      onOk: async () => {
        try {
          ...
          message.success(`Đã xoá ${isTopic ? 'chủ đề' : 'danh mục'} thành công`);
          fetchData();
        } catch (error) {
          message.error(`Lỗi khi xoá ${isTopic ? 'chủ đề' : 'danh mục'}`);
        }
      },
    });
  };
  ...
  }, [categories, topics, activeTab, searchQuery]);

  const tabConfig = [
    { key: 'TOPICS', label: 'Task 2 Topics', color: 'amber' },
    { key: TaskType.TASK_2, label: 'Task 2 Types (Essay)', color: 'violet' },
  ...
  const TASK_TYPE_COLORS: Record<string, string> = {
    ...
    'TOPICS': 'bg-amber-50 text-amber-700 border-amber-200',
  ...
  return (
    <div className="min-h-screen bg-transparent text-slate-900 font-sans selection:bg-orange-500/20 selection:text-orange-900">
              ...
              items={tabConfig.map(t => ({
                ...
                label: (
                  <div className="flex items-center gap-2.5 px-1 pb-4">
                    ...
                    <Badge
                      count={t.key === 'TOPICS' ? topics.length : categories.filter(c => c.taskType === t.key).length}
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-5 border-t border-slate-100/60 mt-[-1px]">
              ...
              <div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-4">
                <div className="flex items-center gap-1.5">
                  ...
                  Tổng số: {activeTab === 'TOPICS' ? topics.length : categories.filter(c => c.taskType === activeTab).length}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 bg-white min-h-[400px]">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-slate-400 text-xs mt-2">Hãy bắt đầu bằng cách tạo một {activeTab === 'TOPICS' ? 'chủ đề' : 'dạng bài'} mới.</p>
              </div>
            ) : activeTab === 'TOPICS' ? (
              /* ── THREE-COLUMN LIST VIEW FOR TOPICS ── */
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 animate-in slide-in-from-bottom-4 duration-500 ease-out">
                {filteredData.map(item => {
                  const theme = getTagColor(item.name);
                  return (
                    <div
                      key={item.id}
                      className="group flex flex-col justify-between bg-white border border-slate-100 hover:border-orange-200 p-5 rounded-[20px] hover:shadow-xl hover:shadow-orange-500/10 transition-all cursor-default"
                    >
                      <div className="flex items-start gap-3 mb-4 min-w-0">
                        <div className={`w-1.5 h-6 rounded-full ${theme.bg} opacity-100 flex-shrink-0 mt-1`}></div>
                        <div className="flex flex-col min-w-0">
                          <h4 className={`text-sm font-black ${theme.text} group-hover:text-orange-600 transition-colors truncate`}>
                            {item.name}
                          </h4>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                        <div className="flex items-center gap-1.5">
                          <div className="w-6 h-6 rounded-lg bg-slate-50 flex items-center justify-center text-[10px] font-black text-slate-700 border border-slate-100">
                            {item.promptsCount || 0}
                          </div>
                          <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">Đề thi</span>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          <Tooltip title="Chỉnh sửa chủ đề">
                            <button
                              ...
                              className="flex items-center justify-center w-9 h-9 bg-slate-50 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition-all border border-slate-100 active:scale-90"
                            >
                              <EditIcon />
                            </button>
                          </Tooltip>
                          <Tooltip title="Xoá vĩnh viễn (CASCADE)">
                            <button
                              onClick={() => handleDelete(item.id, item.name, item.promptsCount || 0)}
                              className="flex items-center justify-center w-9 h-9 bg-red-50 text-red-300 hover:text-red-600 hover:bg-red-100 rounded-xl transition-all border border-red-50 active:scale-90"
                            >
                              <DeleteIcon />
                            </button>
                          </Tooltip>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-500 ease-out">
                {filteredData.map(item => {
                  const theme = getTagColor(item.name);
                  const label = activeTab.replace('_', ' ');
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col h-full">
                        ...
                        <p className="text-slate-400 text-xs line-clamp-2 mt-[-8px] mb-4">
                          {item.description || 'Không có mô tả cho phân loại này.'}
                        </p>
                        ...
                        <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-50">
                          <div className="flex flex-col">
                            ...
                            <span className="text-sm font-black text-slate-700">{item.promptsCount || 0}</span>
                          </div>
      ...
      {/* Modal: Create */}
      <Modal
        title={<span className="text-lg font-bold text-slate-900 tracking-tight">{activeTab === 'TOPICS' ? 'Tạo Chủ đề mới' : 'Tạo Dạng bài mới'}</span>}
        open={isAddModalOpen}
      ...
      >
        <div className="py-6 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Tên hiển thị <span className="text-red-500">*</span></label>
            <AntInput
              value={formName}
              onChange={(e) => setFormName(e.target.value)}
              placeholder={activeTab === 'TOPICS' ? "Ví dụ: Health, Environment, Education..." : "Ví dụ: Opinion, Discussion, Line Graph..."}
      ...
      {/* Modal: Edit */}
      <Modal
        title={<span className="text-lg font-bold text-slate-900 tracking-tight">{activeTab === 'TOPICS' ? 'Sửa Chủ đề' : 'Sửa Dạng bài'}</span>}
      ...
      >
        <div className="py-6 flex flex-col gap-6">
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-slate-400">Tên hiển thị <span className="text-red-500">*</span></label>
            <AntInput
              value={formName}
              onChange={(e) => setFormName(e.target.value)}
              className="h-12 border-slate-200 hover:border-orange-500/50 focus:border-orange-500 shadow-sm rounded-xl font-medium"
