...
// ── Component InputField (Khai báo ngoài RegisterPage để tránh lỗi mất focus liên tục) ──
const InputField: React.FC<{
  ...
  focusedField: string | null; setFocusedField: (id: string | null) => void;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
}> = ({ id, label, type = 'text', placeholder, value, icon, isPassword, showPw, onTogglePw, focusedField, setFocusedField, onChange }) => (
  <div>
    ...
    <div className={`relative rounded-xl border transition-all duration-200 ${focusedField === id ? 'border-indigo-400 ring-4 ring-indigo-500/10 shadow-lg shadow-indigo-100' : 'border-slate-200 hover:border-slate-300'}`}>
      ...
      <input required value={value} onChange={onChange} onFocus={() => setFocusedField(id)} onBlur={() => setFocusedField(null)}
        ...
        <div className="relative z-10 w-full md:w-7/12 p-8 md:p-10 flex flex-col justify-center">
          ...
          <form className="space-y-4" onSubmit={onSubmit}>
            <InputField id="fullName" label="Họ và tên" placeholder="Nguyễn Văn A" value={formData.fullName}
              ...
              focusedField={focusedField} setFocusedField={setFocusedField} onChange={handleChange}
            />
